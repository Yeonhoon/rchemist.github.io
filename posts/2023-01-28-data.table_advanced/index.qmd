---
title: "data.table 심화내용"
description:  "data.table 특수 기호, 조인, 피봇"
author: 
  - name: "JYH"
...
```{r}
dt[,c(.N, lapply(.SD,mean)),by=race,.SDcols='age']
```
:::
...
```{r}
merge(,by, all.x=T) # left join
merge(,by, all.y=T) # right join
```
...
cast는 어떤 형태로 데이터를 굳히는 것을 의미합니다. 기존에 melt로 녹아있던 데이터를 casting 한다고 이해하시면 됩니다.
:::
