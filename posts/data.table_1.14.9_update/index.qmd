...
이제 `data.table`도 `tibble`처럼 열별로 데이터 유형을 확인할 수 있습니다.
...
`DT(i, j, by, …)` 가 추가되었습니다. data.table의 함수적 형태라고 볼 수 있겠습니다.
...
`fifelse()`를 사용할 때 NA와 다른 유형의 데이터가 생성될 수 있게끔 변경되었습니다. 기존에는 `fifelse()`에서 true, false 인자에 들어가는 값에 `NA`가 들어가면 에러 메시지가 떴습니다. 하지만 1.14.9 버전부터는 해당 에러메시지가 뜨지 않고, 정상적으로 잘 출력됩니다.
