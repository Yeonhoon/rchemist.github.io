...
기본적으로 `mice()`는 연속형 변수에 대해서는 `pmm`을, 범주형 변수에 대해서는 `logreg`를 적용해 NA를 채워넣습니다.

범주형 변수에 대해 자동으로 `logreg`를 적용하기 위해서는 해당 변수를 `character` 또는 `factor`로 변경해주어야 합니다.

```{r}
nhanes$hyp <- as.factor(nhanes$hyp)
imp4 <- mice(nhanes, printFlag = F)
imp4
```
...
`log_reg` 방식도 `pmm`과 마찬가지로 진행이 됩니다.
...
더 나은 함수나 패키지가 있는지 확인이 되면 수정하도록 하겠습니다.
:::
