...
```{r}
m1 <- matchit(treat ~ age + educ + race + married + nodegree + re74 + re75, data = lalonde, method="nearest", distance = "glm")
m1
```
...
위의 그래프를 살펴보면, 매칭을 통해 adjusted된 공변량들의 SMD가 0.1 이상인 경우가 꽤 많은 것을 알 수 있습니다. 즉 매칭이 제대로 이루어지지 않은 것이죠.
## 참고자료
